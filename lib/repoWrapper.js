...
module.exports = function(repoUrl, localPath) {
  ...
  RepoWrapper.walkCommits = function(callback, max) {
    return processCommit(firstCommit, callback)
      .then(function () {
        ...
        return checkoutCommit(firstCommit);
      });
  };
  ...
  function processCommit(commit, callback) {
    return new Promise(function(resolve, reject) {
      lock.writeLock(function (release) {
        ...
        return checkoutCommit(commit).then(function() {
            ...
            console.log('Error in commit: ' + commit.sha() + ': ' + err);
            release();  // write lock
            reject(err);
        });
      });
  ...
  function checkoutCommit(commit) {
    ...
    return git.Checkout.tree(gitRepo, commit, checkoutOpts)
      .catch(function(err) {
